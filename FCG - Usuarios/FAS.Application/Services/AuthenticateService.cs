    ...
    public class AuthenticateService : IAuthenticate
    {
        ...
        public async Task<DomainNotificationsResult<UserTokenViewModel>> Login(LoginDTO loginDTO)
        {
            ...
            try
            {
                ...
                if (usuarioRecuperado.Status != StatusProdutor.Ativo)
                {
                    _logger.LogWarning("Tentativa de login de produtor inativo: {email}", loginDTO.EmailUsuario);
                    resultNotifications.Notifications.Add("Conta inativa. Entre em contato com o suporte.");
                    return resultNotifications;
                }
            ...
            catch (Exception ex)
            {
                ...
                resultNotifications.Notifications.Add(ex.InnerException?.Message ?? ex.Message);
            }
        ...
        public async Task<string> GerarToken(int id, string email)
        {
            ...
            var claims = new[]
            {
                ...
                new Claim("ProducerId", id.ToString()), // T-008: outros serviços usam ProducerId/UserId do token
                new Claim("email", email),
                new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()),
                new Claim(ClaimTypes.Role, usuario.IsAdmin ? "Admin" : "Produtor")
            };
